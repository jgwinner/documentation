...
5. Select the `Datadog` destination and select the `Datadog US` or `Datadog EU` region (http endpoint URL), depending on the Datadog Region of your account.
...
1. Check the `Subscription filter` column on the [log groups index page][1] to see current subscriptions to your relevant log groups. Add the new Kinesis stream as a subscriber.    
...
3. Check the `Subscription filters` column in the [log groups index page][1] to confirm that the new Kinesis stream is now subscribed to your log groups.
